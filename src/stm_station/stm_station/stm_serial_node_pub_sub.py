...
class SerialPubSubNode(Node):
    def __init__(self):
        ...
        self.port = '/dev/ttyACM0'  # Serial port name <-- EXERCISE Deliverable 1.2 and 1.3
        ...
        # self.stm_state_topic = '/stm_state' # <-- EXERCISE Deliverable 1.2
        # self.stm_state_publisher = self.create_publisher(Float32MultiArray, self.stm_state_topic, 10) # <-- EXERCISE Deliverable 1.2
        self.stm_state_topic = 'stm_state' # <-- EXERCISE Deliverable 1.3
        self.stm_state_publisher = self.create_publisher(STMState, self.stm_state_topic, 10) # <-- EXERCISE Deliverable 1.3
        ...
        # self.stm_control_topic = '/stm_control' # <-- EXERCISE Deliverable 1.2
        # self.stm_control_subscriber = self.create_subscription(Float32MultiArray, self.stm_control_topic, self.stm_control_callback, 10) # <-- EXERCISE Deliverable 1.2
        # self.stm_control_subscriber  # Prevent unused variable warning <-- EXERCISE Deliverable 1.2
        self.stm_control_topic = 'stm_control' # <-- EXERCISE Deliverable 1.3
        self.stm_control_subscriber = self.create_subscription(STMControl, self.stm_control_topic, self.stm_control_callback, 10) # <-- EXERCISE Deliverable 1.3
        self.stm_control_subscriber  # Prevent unused variable warning <-- EXERCISE Deliverable 1.3
        ...
        if self.serial_port.in_waiting > 0:
            try:
                ...
                if len(float_values) == 8:
                    ...
                    # float32_multi_array_msg = Float32MultiArray() # <-- EXERCISE Deliverable 1.2
                    # float32_multi_array_msg.data = [float(float_values[0]),float(float_values[1]),float(float_values[2]),float(float_values[3]),
                    #     float(float_values[4]),float(float_values[5]),float(float_values[6]),float(float_values[7])] # <-- EXERCISE Deliverable 1.2
                    # Create a STMState message
                    # TODO: Create and populate the STMState message
                    STMState_msg = STMState() # <-- EXERCISE Deliverable 1.3
                    STMState_msg.motor_encoder = float(float_values[0]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.motor_velocity = float(float_values[1]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.accel_x = float(float_values[2]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.accel_y = float(float_values[3]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.accel_z = float(float_values[4]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.gyro_x = float(float_values[5]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.gyro_y = float(float_values[6]) # <-- EXERCISE Deliverable 1.3
                    STMState_msg.gyro_z = float(float_values[7]) # <-- EXERCISE Deliverable 1.3

                    # # Publish the message
                    # # TODO: Publish the float32_multi_array_msg
                    # self.stm_state_publisher.publish(float32_multi_array_msg) # <-- EXERCISE Deliverable 1.2
                    # Publish the message
                    # TODO: Publish the STMState_msg
                    self.stm_state_publisher.publish(STMState_msg) # <-- EXERCISE Deliverable 1.3
    ...
    def stm_control_callback(self, msg):
        ...
        try:
            ...
            # control_data = f"{{{msg.data[0]}, {msg.data[1]}, {msg.data[2]}, {msg.data[3]}}}" # <-- EXERCISE Deliverable 1.2
            # TODO: Format the control data as a comma-separated string and send it to the serial device
            # format: {control_type, goal_position, Kp, PWM}
            # control_type: 0 = Stop, 1 = Position Control (Close Loop), 2 = PWM Control (Open Loop)
            control_data = f"{{{msg.control_type}, {msg.setpoint}, {msg.kp}, {msg.pwm}}}" # <-- EXERCISE Deliverable 1.3
