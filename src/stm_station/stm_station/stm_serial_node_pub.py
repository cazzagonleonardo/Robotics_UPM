...
class SerialPublisherNode(Node):
    def __init__(self):
        ...
        self.port = '/dev/ttyACM0'  # Serial port name <-- EXERCISE 4
        ...
        self.stm_state_publisher = self.create_publisher(Float32MultiArray, self.stm_state_topic, 10) # <-- EXERCISE 4
            ...
            try:
                ...
                if len(float_values) == 8:
                    ...
                    float32_multi_array_msg.data = [float(float_values[0]),float(float_values[1]),float(float_values[2]),float(float_values[3]),
                        float(float_values[4]),float(float_values[5]),float(float_values[6]),float(float_values[7])] # <-- EXERCISE 4

                    # Publish the message
                    # TODO: Publish the float32_multi_array_msg
                    self.stm_state_publisher.publish(float32_multi_array_msg) # <-- EXERCISE 4
